...
if (output?.type !== mockImports['./components/LoginPage']) {
  console.error("FAIL: Expected mock LoginPage component, got", output?.type);
  process.exit(1);
} else {
  console.log("PASS");
}
...
// Wait a tick for mount effects (like initAuth registration) to run
process.nextTick(() => {
  loginPageProps2.onSignIn().then(() => {
    // Now triggerUser is definitely defined!
    ...
    setTimeout(() => {
      ...
      const searchTree = (node: any) => {
        if (!node) return;
        if (node.type === mockImports['./components/Dashboard']) {
          dashboardElement = node;
          return;
        }
        if (node.children) {
        ...
        if (node.props && node.props.children) {
          if (Array.isArray(node.props.children)) {
            node.props.children.forEach(searchTree);
          } else {
            searchTree(node.props.children);
          }
        }
      };
      searchTree(output);

      if (dashboardElement && dashboardElement.props?.rows?.[0]?.bookingId === 'LIVE-1') {
        console.log("PASS: Loaded live rows!");
      } else {
        console.error("FAIL: Dashboard rows not populated with live rows. DashboardElement:", dashboardElement);
        process.exit(1);
      }
      ...
      // Wait a tick for mount effects
      process.nextTick(() => {
        const loginPageProps3 = output.props;
        loginPageProps3.onSignIn().then(() => {
          // Trigger auth listener with error-token (causes 403 error in fetchSheetDataDirect)
          ...
          setTimeout(() => {
            ...
            const hasLockIcon = stringified.includes('Lock') || stringified.includes('LockIcon');
            const hasRestrictedMsg = stringified.includes('Access Restricted');
            
            let dashboardFound = false;
            dashboardElement = null;
            searchTree(output);
            if (dashboardElement) dashboardFound = true;

            console.log("Dashboard rendered:", dashboardFound);
            console.log("Has Restricted Access Message:", hasRestrictedMsg);
            console.log("Has Lock Icon:", hasLockIcon);
            
            if (hasRestrictedMsg && !dashboardFound) {
              console.log("PASS: Correctly rendered Restricted Access page and did not render Dashboard.");
            } else {
              console.error("FAIL: Did not show Restricted Access screen or Dashboard was incorrectly rendered.");
              process.exit(1);
            }
